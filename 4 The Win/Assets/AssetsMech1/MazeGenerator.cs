        ...
        for (int i = 0; i < size+1; i++)
        {
            for (int j = 0; j < size+1; j++)
            {
                if( hwalls[i,j] && i < size){
                    var prefab = Instantiate(wall, new Vector3(i*scale, j*scale-scale/2,0 ), Quaternion.identity);
                ...
                if( vwalls[i,j] && j < size ){
                    var prefab = Instantiate(wall, new Vector3(i*scale-scale/2, j*scale,0 ), Quaternion.identity);
